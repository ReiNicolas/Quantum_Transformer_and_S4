...
class Optimizer:
    def __init__(self, model, Hamiltonians, point_of_interest=None,ckpt_freq = 10000):
        self.model = model
    ...
    def train(self, n_iter, batch=10000, max_unique=1000,
        ...
        name, embedding_size = type(self.Hamiltonians[0]).__name__,\
        ...
        save_str = f'S4_{name}_{embedding_size}_{ensemble_id}' if not fine_tuning\
            else f'ft_{self.model.system_sizes[0].detach().cpu().numpy().item()}_' \
                 f'{param_range[0].detach().cpu().numpy().item():.2f}_' \
                 f'{name}_{embedding_size}_{ensemble_id}'
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(
            optim,
            lambda step: self.lr_schedule(step, self.model.hidden_size, start_step=start_iter))
